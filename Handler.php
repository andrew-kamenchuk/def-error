...
class Handler
{
	...
	protected $includeTrace = false, $traceOptions = \DEBUG_BACKTRACE_PROVIDE_OBJECT;
	...
	public function off($error, callable $handler = null)
	{
		...
		while($j <<= 1 < \E_ALL) if($j & $error)
			if(!isset($handler))
				$this->handlers[$j] = [];
			elseif(false !== $key = \array_search($handler, $this->handlers[$j]))
	...
	public function handle($type, $message, $file = null, $line = null, array $context = null)
	{
		if($type & $this->errorReporting & \error_reporting()) {
			...
			foreach($this->handlers[$type] as $handler)
				if(true === $handler($type, $message, $file, $line, $context, $trace)) return true;
		}
	...
	public function register()
	{
		...
		\register_shutdown_function(function() use(&$reservedMemory) {
			...
			if($e = \error_get_last() and ($e['type'] & (\E_PARSE | \E_ERROR | \E_CORE_ERROR | \E_COMPILE_ERROR | \E_USER_ERROR))) {
	...
	{
		return $this->on($mask, function($type, $message, $file = null, $line = null, array $context = null, array $trace) use($logger, $level) {
			return $logger->log($level, $message, ['error' => \compact('type', 'message', 'line', 'file', 'context', 'trace')]);
		});
	}
